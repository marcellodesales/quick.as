	...
	Q.fcall(validate)
		...
		.then(function(){
			utilities.cryptPassword(password,function(err,pwd){
				client.query("INSERT INTO users(created,firstname,lastname,email,username,password,mailinglist) values ($1,$2,$3,$4,$5,$6,$7) WHERE NOT EXISTS ( SELECT username FROM users WHERE username = $4 )", [new Date(), firstname, lastname, email, username, pwd, mailinglist])
					.on('end', function(){
						client.end();
